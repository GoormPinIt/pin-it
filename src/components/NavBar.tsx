...
import { AiOutlineMessage, AiOutlineHome,AiOutlineSetting } from "react-icons/ai";
...
const NavBar:React.FC = () => {
    ...
    const toggleModal = (type : string, title : string, content : React.ReactNode)=>{
    ...
    const handleHomeClick = () => {
      ...
      const handleUpdateClick  = () => {
        toggleModal("update", "업데이트", <p>바이</p>);
};
    
      const handleMessageClick = () => {
        toggleModal("message", "메시지", <p>안녕녕</p>);
    };
    ...
    return (
        <div className="sticky top-0 left-0  w-10 h-screen bg-gray-100 flex flex-col border-r-2">
            <div className='flex-1 flex flex-col'>
                ...
                <button
                className='px-3 py-4' onClick={handleCreateClick}
                ><FaRegPlusSquare /></button>
                <button
                className='px-3 py-4' onClick={handleUpdateClick}
                ><FaRegBell /></button>
                <button
                className='px-3 py-4' onClick={handleMessageClick}
                ><AiOutlineMessage /></button>
            </div>
            <div>
            <button
                className='px-3 py-2' onClick={handleSettingClick}
