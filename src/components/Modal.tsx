...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, title, messages }) => {
  ...
  <div className="fixed top-0 left-12 w-full h-full rounded-lg bg-opacity-50 z-50 flex justify-start items-center">
    <div className="w-[180px] h-[98%] bg-white shadow-lg rounded-md">
        {liveMessage ? (<LiveMessage sender={liveMessage} onClose={closeChatRoom} />):(
      <div>
        ...
        <div className="text-xs flex p-2"><button className="bg-red-600 rounded-full w-5 h-5 items-center justify-center">
          <FaRegPenToSquare
          style={{ fontSize: '12px', lineHeight: '1'}}
          className="text-[15px] ml-1"  color="#fff"/></button>새 메시지</div>
        </div>
            ...
            <div key={index}
                 ...
                 className="flex cursor-pointer items-center justify-between pb-2 mb-2 hover:bg-gray-300 rounded-lg">
                  <div className="flex items-center gap-2 ">
                    ...
                    <div>
                      <div className="font-bold text-[10px]">{msg.sender}</div>
                      <div className="text-[9px] text-gray-400">{msg.text}</div>
                    </div>
                  </div>
